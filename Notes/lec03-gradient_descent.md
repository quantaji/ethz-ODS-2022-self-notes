...
- The goal is to find approximation $\mathbf{x}$, s.t. $f(\mathbf{x})-f(\mathbf{x}^{\star})<\varepsilon$. 
...
- **Lemma A (generalized Cauchy–Schwarz inequality)** $\nabla f$ is $L$-Lipschitz $arrow$ $\forall \mathbf{x,y}\in\mathbf{dom}(f)$, $(\nabla f(x)-\nabla f(y))^{T}(x-y) \leq L\Vert x-y\Vert ^{2}_{a}$
    - **Proof**
        - $(\nabla f(x)-\nabla f(y))^{T}(x-y) \leq \Vert \nabla f(x)-\nabla f(y)\Vert _{a*}\Vert x-y\Vert _{a} \leq L\Vert x-y\Vert _a^2$
    
### Smoothness
- **Definition 3.2 (Smoothness)** Let $f: \operatorname{dom}(f) arrow \mathbb{R}$ be a differentiable function, $X \subset \operatorname{dom}(f)$ convex and $L \in \mathbb{R}_{+}$. Function $f$ is called *smooth with parameter $L$* over $X$ (over norm $\Vert \cdot\Vert _a$) if $f(\mathbf{y}) \leq f(\mathbf{x})+\nabla f(\mathbf{x})^{\top}(\mathbf{y}-\mathbf{x})+\frac{L}{2}\Vert \mathbf{x}-\mathbf{y}\Vert _{a}^{2}$
    - PS: **NO** need to assume $f$ to be convex
- **Lemma 3.3 (equivalance of smoothness in 2-norm)** $f$ is $L$-smooth over 2-norm  $rightarrow$ $g(\mathbf{x})=\frac{L}{2} \mathbf{x}^{\top} \mathbf{x}-f(\mathbf{x})$ is convex over $\operatorname{dom}(f)$.
    - **Proof** 
        - $\nabla g(\mathbf{x}) = L\mathbf{x} - \nabla f(\mathbf{x})$
        - $g$ convex $rightarrow$ $\forall \mathbf{x,y}, g(\mathbf{y}) \geq g(\mathbf{x}) + \nabla g(\mathbf{x})^{\top}(\mathbf{y-x})$ $rightarrow$ $\frac{L}{2} \mathbf{y}^{\top} \mathbf{y}-f(\mathbf{y}) \geq \frac{L}{2} \mathbf{x}^{\top} \mathbf{x}-f(\mathbf{x}) + [ L\mathbf{x} - \nabla f(\mathbf{x}) ]^{\top}(\mathbf{y-x})$ $rightarrow$ $f(\mathbf{y}) \leq f(\mathbf{x})+\nabla f(\mathbf{x})^{\top}(\mathbf{y}-\mathbf{x})+\frac{L}{2}\Vert \mathbf{x}-\mathbf{y}\Vert _{2}^{2}$
    - If $f$ twice differentiable, Hessian of $g$ is $LI-\nabla^2 f \succeq 0$, this means $\forall x, \lambda_{\max }(\nabla^{2} f(x)) \leq L$.
- **Lemma B (quadratic upper bound)** If $\operatorname{dom} f=\mathbf{R}^{n}$ and $f$ is $L$-smooth over norm $a$, and has minimizer $\mathbf{x}^{\star}$, then $\forall z$, $\frac{1}{2 L}\Vert \nabla f(z)\Vert _{a*}^{2} \leq f(z)-f(x^{\star}) \leq \frac{L}{2}\lVert z-x^{\star}\rVert_{a}^{2}$
    - **Proof**
        ...
        - Left-hand ineuqality, by smoothness $\inf _{y} f(y) \leq \inf _{y}(f(z)+\nabla f(z)^{T}(y-z)+\frac{L}{2}\Vert y-z\Vert ^{2})$
            - by seperation of direction and magnitude $\mathsf{LHS }\leq =\inf _{\Vert v\Vert =1} \inf _{t}(f(z)+t \nabla f(z)^{T} v+\frac{L t^{2}}{2}) = \inf _{\Vert v\Vert =1}(f(z)-\frac{1}{2 L}(\nabla f(z)^{T} v)^{2})$ by maximum of quadratic function.
...
- **Lemma 3.5 (Equivalence of Smoothness and Lipschitz under convexity)** If (i) $\mathbf{dom}(f) = \mathbb{R}^{d}$ (ii) $f$ convex and differentiable. Then $f$ is smooth with parameter $L$ under norm $a$ $rightarrow$ $\nabla f$ is Lipschitz continuous with parameter $L$ under norm $a$.
...
-  **Lemma E (quadratic lower bound)** If $f$ is closed (means has closed sublevel sets), and $\mathbf{x}^{\star}$ is its unique minimizer, then $\forall z\in \mathbf{dom}(f), \frac{m}{2}\lVert z-x^{\star}\rVert^{2} \leq f(z)-f(x^{\star}) \leq \frac{1}{2 m}\Vert \nabla f(z)\Vert _{*}^{2}.$
...
## Speed metric
- $\lim _{k arrow \infty} \frac{f(w^{k+1})-f(w^{*})}{f(w^{k})-f(w^{*})}=\rho$
...
- Consider quantity $\mathbf{g}_{t}^{\top}(\mathbf{x}_{t}-\mathbf{x}^{\star})=\frac{1}{\gamma}(\mathbf{x}_{t}-\mathbf{x}_{t+1})^{\top}(\mathbf{x}_{t}-\mathbf{x}^{\star}) = \frac{1}{2 \gamma}(\lVert\mathbf{x}_{t}-\mathbf{x}_{t+1}\rVert^{2}+\lVert\mathbf{x}_{t}-\mathbf{x}^{\star}\rVert^{2}-\lVert\mathbf{x}_{t+1}-\mathbf{x}^{\star}\rVert^{2}) = \frac{\gamma}{2}\lVert\mathbf{g}_{t}\rVert^{2}+\frac{1}{2 \gamma}(\lVert\mathbf{x}_{t}-\mathbf{x}^{\star}\rVert^{2}-\lVert\mathbf{x}_{t+1}-\mathbf{x}^{\star}\rVert^{2})$
- sum over $t=0,\ldots,T-1$, we have $\sum_{t=0}^{T-1} \mathbf{g}_{t}^{\top}(\mathbf{x}_{t}-\mathbf{x}^{\star})=\frac{\gamma}{2} \sum_{t=0}^{T-1}\lVert\mathbf{g}_{t}\rVert^{2}+\frac{1}{2 \gamma}(\lVert\mathbf{x}_{0}-\mathbf{x}^{\star}\rVert^{2}-\lVert\mathbf{x}_{T}-\mathbf{x}^{\star}\rVert^{2}) \leq =\frac{\gamma}{2} \sum_{t=0}^{T-1}\lVert\mathbf{g}_{t}\rVert^{2}+\frac{1}{2 \gamma}\lVert\mathbf{x}_{0}-\mathbf{x}^{\star}\rVert^{2}$
- By convexity $f(\mathbf{x}_{t})-f(\mathbf{x}^{\star}) \leq \mathbf{g}_{t}^{\top}(\mathbf{x}_{t}-\mathbf{x}^{\star})$, we have $\sum_{t=0}^{T-1}(f(\mathbf{x}_{t})-f(\mathbf{x}^{\star})) \leq \frac{\gamma}{2} \sum_{t=0}^{T-1}\lVert\mathbf{g}_{t}\rVert^{2}+\frac{1}{2 \gamma}\lVert\mathbf{x}_{0}-\mathbf{x}^{\star}\rVert^{2}$
    - This gives upper bound on average error $\mathbb{E}_{t} [f(\mathbf{x}_{t})-f(\mathbf{x}^{\star})]$
...
- **Theorem 3.1** Suppose $f \in C^1(x)$ convex, with global minimum $\mathbf{x}^{\star}$. If (1) $\lVert\mathbf{x}_{0}-\mathbf{x}^{\star}\rVert \leq R$ (2) $\forall \mathbf{x}, \Vert \nabla f(\mathbf{x})\Vert  \leq B$, when with step size $\gamma:=\frac{R}{B \sqrt{T}}$, we have average error bounded by $\frac{1}{T} \sum_{t=0}^{T-1}(f(\mathbf{x}_{t})-f(\mathbf{x}^{\star})) \leq \frac{R B}{\sqrt{T}}$
    - **Proof** Straighforward from vanilla case, $\sum_{t=0}^{T-1}(f(\mathbf{x}_{t})-f(\mathbf{x}^{\star})) \leq \frac{\gamma}{2} B^{2} T+\frac{1}{2 \gamma} R^{2}$
    - **Time Complexity** To achieve $\min _{t=0}^{T-1}(f(\mathbf{x}_{t})-f(\mathbf{x}^{\star})) \leq \varepsilon$, we need $T \geq \frac{R^{2} B^{2}}{\varepsilon^{2}}$ iterations, $\mathcal{O}(1 / \varepsilon^{2})$.
    - but no dependence on dimension $d$.

## Improvement Case 2: Smooth Convex $f$: $\mathcal{O}(1 / \varepsilon)$ Steps
- **Lemma 3.7 (Sufficient Descent)** Suppose $f: \mathbb{R}^{d} arrow \mathbb{R} \in C^{1}$ is $L$-smooth. Setting $\gamma := L^{-1}$, then gradient descent gives $f(\mathbf{x}_{t+1}) \leq f(\mathbf{x}_{t})-\frac{1}{2 L}\lVert\nabla f(\mathbf{x}_{t})\rVert^{2}$. (**Proof** Straightforward)
- **Lemma 3.8 (Last Iteration Bound for smooth convex funciton)** Suppose $f: \mathbb{R}^{d} arrow \mathbb{R} \in C^{1}$ is $L$-smooth with global minimum $\mathbf{x}^{\star}$. With step size $\gamma := L^{-1}$ gradient descent gives $f(\mathbf{x}_{T})-f(\mathbf{x}^{\star}) \leq \frac{L}{2 T}\lVert\mathbf{x}_{0}-\mathbf{x}^{\star}\rVert^{2}$.
    - **Proof** 
        - Sum *sufficient descent* over $t=0:T-1$, we get $\frac{1}{2 L} \sum_{t=0}^{T-1}\lVert\nabla f(\mathbf{x}_{t})\rVert^{2} \leq \sum_{t=0}^{T-1}(f(\mathbf{x}_{t})-f(\mathbf{x}_{t+1}))=f(\mathbf{x}_{0})-f(\mathbf{x}_{T})$
        - Take this into vanilla GD bound, we get $\sum_{t=1}^{T}(f(\mathbf{x}_{t})-f(\mathbf{x}^{\star})) \leq \frac{L}{2}\lVert\mathbf{x}_{0}-\mathbf{x}^{\star}\rVert^{2}$.
        - By sufficient descent, monotocity of $f(\mathbf{x}_{t})-f(\mathbf{x}^{\star})$ is guaranteed, so $f(\mathbf{x}_{T})-f(\mathbf{x}^{\star}) \leq \frac{1}{T} \sum_{t=1}^{T}(f(\mathbf{x}_{t})-f(\mathbf{x}^{\star})) \leq \frac{L}{2 T}\lVert\mathbf{x}_{0}-\mathbf{x}^{\star}\rVert^{2}$
...
- **Theorem (Nesterov Acceleration)** Suppose $f\in C^{1}(\mathbb{R}^{d} arrow \mathbb{R})$ is convex  and $L$-smooth with global minimum $\mathbf{x}^{\star}$. Then Nesterov's Accelerated gradient descent algorithm gives $f(\mathbf{y}_{T})-f(\mathbf{x}^{\star}) \leq \frac{2 L\lVert\mathbf{z}_{0}-\mathbf{x}^{\star}\rVert^{2}_{2}}{T(T+1)}$.
    - **Proof** (*potential function argument*)
        - Define potential function $\Phi(t):=t(t+1)(f(\mathbf{y}_{t})-f(\mathbf{x}^{\star}))+2 L\lVert\mathbf{z}_{t}-\mathbf{x}^{\star}\rVert_{2}^{2}$, we aim to show that $\Phi(T) \leq \Phi(0)$, then we can prove this theorem.
        - Define $\Delta:=\frac{\Phi(t+1)-\Phi(t)}{t+1}$, and we need to show that it's less than zero
            - $\Delta  =  \frac{1}{t+1}[(t+2)(t+1)(f(\mathbf{y}_{t+1})-f(\mathbf{x}^{\star})) - t(t+1)(f(\mathbf{y}_{t})-f(\mathbf{x}^{\star}))  + 2 L(\lVert\mathbf{z}_{t+1}-\mathbf{x}^{\star}\rVert^{2}_{2} - \lVert\mathbf{z}_{t}-\mathbf{x}^{\star}\rVert^{2}_{2})]$
            - $=t(f(\mathbf{y}_{t+1})-f(\mathbf{y}_{t}))+2(f(\mathbf{y}_{t+1})-f(\mathbf{x}^{\star}))+\frac{2 L}{t+1}(\lVert\mathbf{z}_{t+1}-\mathbf{x}^{\star}\rVert_{2}^{2}-\lVert\mathbf{z}_{t}-\mathbf{x}^{\star}\rVert_{2}^{2})$
            - Since $\mathbf{g}_{t}=\frac{2L}{t+1}(\mathbf{z}_{t} - \mathbf{z}_{t+1})$, we consider $\mathbf{g}_{t}^{\top}(\mathbf{z}_{t}-\mathbf{x}^{\star}) = \frac{2L}{t+1}(\mathbf{z}_{t} - \mathbf{z}_{t+1})^{\top}(\mathbf{z}_{t}-\mathbf{x}^{\star})$
            - by $v^{\top}w = \frac{\Vert v\Vert _{2}^2 + \Vert w\Vert _{2}^2 - \Vert v-w\Vert _{2}^2}{2}$, we have $\mathbf{g}_{t}^{\top}(\mathbf{z}_{t}-\mathbf{x}^{\star}) = \frac{t+1}{4L} \Vert \mathbf{g}_t\Vert _{2}^2  +    \frac{L}{t+1}( \Vert \mathbf{z}_{t}-\mathbf{x}^{\star}\Vert _{2}^2 - \Vert \mathbf{z}_{t+1}-\mathbf{x}^{\star}\Vert _{2}^2 )$
            - plug this into $\Delta$ we get $\Delta = t(f(\mathbf{y}_{t+1})-f(\mathbf{y}_{t}))+2(f(\mathbf{y}_{t+1})-f(\mathbf{x}^{\star})) - 2\mathbf{g}_{t}^{\top}(\mathbf{z}_{t}-\mathbf{x}^{\star})  + \frac{t+1}{2L} \Vert \mathbf{g}_t\Vert ^2_{2}$
            - By sufficient descent of $y_{t+1}$, $f(y_{t+1})\leq f(x_{t})- \frac{1}{2L}\Vert \mathbf{g}_t\Vert ^2_{2}$, we replace $f(y_{t+1})$ w.r.t $f(x_{t})$ and ommit term $- \frac{1}{2L} \Vert \mathbf{g}_t\Vert ^2_{2}$
                - $\Delta \leq t(f(\mathbf{x}_{t})-f(\mathbf{y}_{t}))+2(f(\mathbf{x}_{t})-f(\mathbf{x}^{\star})) - 2\mathbf{g}_{t}^{\top}(\mathbf{z}_{t}-\mathbf{x}^{\star})$
                - 
            - By convexity, $\Delta \leq t\mathbf{g}_{t}^{\top}(\mathbf{x}_{t} - \mathbf{y}_t) + 2\mathbf{g}_{t}^{\top}(\mathbf{x}_{t} - \mathbf{x}^{\star}) - 2\mathbf{g}_{t}^{\top}(\mathbf{z}_{t}-\mathbf{x}^{\star}) = \mathbf{g}_{t}^{\top}[ (t+2)\mathbf{x}_{t} - t \mathbf{y}_t  - 2\mathbf{z}_{t} ] = 0$ by definition of $\mathbf{x}_{t}$ .
       
## Improvement Case 4: Smooth and Strongly convex $f$: $\mathcal{O}(\log (1 / \varepsilon))$ Steps, linear rate
- **Theorem 3.14 (Linear rate for smooth and strongly convex function)** Suppose $f\in C^{1}(\mathbb{R}^{d} arrow \mathbb{R})$ is $L$-smooth and $\mu$-strongly convex with global minimum $\mathbf{x}^{\star}$. Choosing step size of $\gamma := L^{-1}$, then gradient descent with arbitrary $\mathbf{x}_0$,
    - (i) Geometric decrease for $\lVert\mathbf{x}_{t}-\mathbf{x}^{\star}\rVert^{2}$, $\lVert\mathbf{x}_{t+1}-\mathbf{x}^{\star}\rVert^{2} \leq(1-\frac{\mu}{L})\lVert\mathbf{x}_{t}-\mathbf{x}^{\star}\rVert^{2}$
    - (ii) Exponential decrease for absoulute error $f(\mathbf{x}_{T})-f(\mathbf{x}^{\star}) \leq \frac{L}{2}(1-\frac{\mu}{L})^{T}\lVert\mathbf{x}_{0}-\mathbf{x}^{\star}\rVert^{2}$
    - **Proof**
        - By storng convexity, $\mathbf{g}_{t}^{\top}(\mathbf{x}_{t}-\mathbf{x}^{\star})=\nabla f(\mathbf{x}_{t})^{\top}(\mathbf{x}_{t}-\mathbf{x}^{\star}) \geq f(\mathbf{x}_{t})-f(\mathbf{x}^{\star})+\frac{\mu}{2}\lVert\mathbf{x}_{t}-\mathbf{x}^{\star}\rVert^{2}$
            - again by the fact $\mathbf{g}_{t}^{\top}(\mathbf{x}_{t}-\mathbf{x}^{\star}) = \frac{1}{\gamma}(\mathbf{x}_{t} - \mathbf{x}_{t+1})^{\top}(\mathbf{x}_{t}-\mathbf{x}^{\star}) = \frac{1}{2\gamma}(\gamma^2\Vert \mathbf{g}_{t}\Vert ^2 + \Vert \mathbf{x}_{t}-\mathbf{x}^{\star}\Vert ^2 - \Vert \mathbf{x}_{t+1}-\mathbf{x}^{\star}\Vert ^2)$
            - so that $f(\mathbf{x}_{t})-f(\mathbf{x}^{\star}) \leq \frac{1}{2\gamma}(\gamma^2\Vert \mathbf{g}_{t}\Vert ^2 + \Vert \mathbf{x}_{t}-\mathbf{x}^{\star}\Vert ^2 - \Vert \mathbf{x}_{t+1}-\mathbf{x}^{\star}\Vert ^2) - \frac{\mu}{2}\lVert\mathbf{x}_{t}-\mathbf{x}^{\star}\rVert^{2}$
            - Equivalently, $\Vert \mathbf{x}_{t+1}-\mathbf{x}^{\star}\Vert ^2 \leq (1-\mu\gamma)\Vert \mathbf{x}_{t}-\mathbf{x}^{\star}\Vert ^2 + \gamma^2\Vert \mathbf{g}_{t}\Vert ^2 - 2\gamma (f(\mathbf{x}_{t})-f(\mathbf{x}^{\star}))$
        - By sufficient decrease, $f(\mathbf{x}^{\star})-f(\mathbf{x}_{t}) \leq f(\mathbf{x}_{t+1})-f(\mathbf{x}_{t}) \leq-\frac{1}{2 L}\lVert\nabla f(\mathbf{x}_{t})\rVert^{2}$
            - so that $\gamma^2\Vert \mathbf{g}_{t}\Vert ^2 - 2\gamma (f(\mathbf{x}_{t})-f(\mathbf{x}^{\star})) \leq 0$
        ...
        - Iteratively, $\lVert\mathbf{x}_{T}-\mathbf{x}^{\star}\rVert^{2} \leq(1-\frac{\mu}{L})^{T}\lVert\mathbf{x}_{0}-\mathbf{x}^{\star}\rVert^{2}$
        - By Strong convexity, $f(\mathbf{x}_{T})-f(\mathbf{x}^{\star}) \leq \nabla f(\mathbf{x}^{\star})^{\top}(\mathbf{x}_{T}-\mathbf{x}^{\star})+\frac{L}{2}\lVert\mathbf{x}_{T}-\mathbf{x}^{\star}\rVert^{2}=\frac{L}{2}\lVert\mathbf{x}_{T}-\mathbf{x}^{\star}\rVert^{2}$.
    - **Time Complexiy** $T \geq \frac{L}{\mu} \ln (\frac{R^{2} L}{2 \varepsilon}) = \mathcal{O}(\log (1 / \varepsilon))$
